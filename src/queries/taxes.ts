...
export const useTaxes = (userId :string, year :number | undefined, month :number | undefined, modality :TradeModality, token :string | undefined) => {
    return useQuery<TaxesI>({
        queryKey: taxesKeys.list(userId, year, month, modality),
        queryFn: ()=> taxesService.getInfo(userId, year, month, modality, token),
        staleTime: 1000 * 60 * 5,
    })
}