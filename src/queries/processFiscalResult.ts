...
export const useProcessFiscalResult = () => {
    return useMutation<ProcessFiscalResultResponse , Error, InputInfo>({
        mutationFn: ({ userId, selectedYear, selectedMonth: month, tradeModality: modality } :InputInfo) => processFiscalResultService.execute(userId, selectedYear, month, modality),
        onSuccess: (createdProcessFiscalResult, {userId, selectedYear, selectedMonth, tradeModality}) => {
            ...
            createdProcessFiscalResult && queryClient.setQueryData(processFiscalResultKeys.create(userId, selectedYear, selectedMonth, tradeModality), createdProcessFiscalResult)
        },
        onError: (error) => {
